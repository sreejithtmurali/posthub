...
class HomeViewModel extends BaseViewModel {
  ...
  void deleteItem(String itemId, int index) {
    // Get reference to the 'myadds' node
    DatabaseReference db_Ref = FirebaseDatabase.instance.ref().child('myadds');

    // Use the 'remove' method to delete the item with the specified itemId
    db_Ref.child(itemId).remove().then((_) {
  ...
  void fetchDataFromFirebase() {
    ...
    db_Ref.onValue.listen((DatabaseEvent event) {
      ...
      print('workers::::${postlist.toString()}');
    });
  }
